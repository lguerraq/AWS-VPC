...
resource "aws_default_security_group" "this" {
  ...
  tags = merge(
    { "Name" = coalesce(var.vpc_name,var.default_security_group_name) },
    var.tags_project,
  )
}
...
resource "aws_default_route_table" "default" {
  ...
  timeouts {
    ...
    tags = merge(
    { "Name" = coalesce(var.vpc_name,var.default_route_table_name,"-rt")},
    var.tags_project,
  )
}
...
resource "aws_subnet" "public" {
  ...
  availability_zone               = length(regexall("^[A-Za-z0-9]", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[A-Za-z0-9]", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
...
resource "aws_db_subnet_group" "database" {
  ...
  subnet_ids  = aws_subnet.database[*].id
    ...
    tags = merge(
    {
      "Name" = format( "${var.vpc_name}-${var.database_subnet_group_name}-%s",element(var.azs, count.index),)
    },
    var.tags_project,
  )
}
...
resource "aws_network_acl_rule" "acl_public_inbound_rule" {
...
resource "aws_network_acl_rule" "acl_public_outbound_rule" {
...
resource "aws_network_acl" "database" {
  ...
  tags = merge(
    { "Name" = "${var.vpc_name}-${var.database_subnet_suffix}-acl" },
    var.tags_project,
  )
}

resource "aws_network_acl_rule" "acl_database_inbound_rule" {
...
resource "aws_default_vpc" "this" {
  ...
  enable_dns_hostnames = var.default_vpc_enable_dns_hostnames
    tags = merge(
    { "Name" = coalesce(var.vpc_name,var.default_vpc_name) },
    var.tags_project,
  )
}
